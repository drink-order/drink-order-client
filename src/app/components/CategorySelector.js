...
const CategorySelector = ({ categories = [] }) => {
  const [activeCategory, setActiveCategory] = useState(0); // Default active category is the first
  const [selectedDrink, setSelectedDrink] = useState(null); // Track selected drink for details

  const handleCategoryClick = (categoryIndex) => {
    setActiveCategory(categoryIndex); // Update active category index
    setSelectedDrink(null); // Reset selected drink when category changes
  };

  const handleCardClick = (drink) => {
    setSelectedDrink(drink); // Set the selected drink for details view
  };
  ...
  return (
    <div className="w-full">
      ...
      {/* Conditionally show either the StickyCardComponent or category products */}
      {selectedDrink ? (
        <StickyCardComponent
          image={selectedDrink.image}
          title={selectedDrink.title}
          soldCount={selectedDrink.soldCount}
          price={selectedDrink.price}
          description={selectedDrink.description}
          onBack={() => setSelectedDrink(null)} // Back button handler
        />
      ) : (
        <div className="flex justify-center px-2">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 w-full max-w-4xl">
